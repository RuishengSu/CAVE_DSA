...
def prepare_masks(row, mode):  # mode = 'train', 'val', or 'test'
    ...
    artery_annotations = natsorted(glob.glob(
        os.path.join(input_files.raw_artery_annotation_dir, patient_id,
    ...
    if len(artery_annotations) != 0:
        ...
        dst_artery_mask_path = os.path.join(input_files.prepared_data_out_dir, mode, 'masks_artery',
    ...
    vein_annotations = natsorted(glob.glob(
        os.path.join(input_files.raw_vein_annotation_dir, patient_id,
                     '{}-{}-{}-labels.png'.format(row.filename, '[0-9]'*8, '[0-9]'*6))))
    if len(vein_annotations) != 0:
        ...
        dst_vein_mask_path = os.path.join(input_files.prepared_data_out_dir, mode, 'masks_vein',
                                          '{}_{}.png'.format(patient_id, row.filename))
    ...
    if len(artery_annotations) * len(vein_annotations) != 0:
        ...
        dst_vessel_mask_path = os.path.join(input_files.prepared_data_out_dir, mode, 'masks_vessel',
                                            '{}_{}.png'.format(patient_id, row.filename))
    ...
    if len(artery_annotations) * len(vein_annotations) != 0:
        ...
        dst_av_mask_path = os.path.join(input_files.prepared_data_out_dir, mode, 'masks_av',
                                        '{}_{}.png'.format(patient_id, row.filename))
...
if __name__ == '__main__':
    ...
    df_info = pd.read_csv(input_files.patient_info_csv)
